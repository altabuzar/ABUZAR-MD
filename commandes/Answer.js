...
zokou({nomCom:"abuzar",reaction:"📡",categorie:"IA"},async(dest,zk,commandeOptions)=>{

  const {repondre,ms,arg}=commandeOptions;
  
    if(!arg || !arg[0])
    {return repondre("🐞HI IT'S YOUR FAVORITE DEVELOPER ABUZAR🧋.")}
    //var quest = arg.join(' ');
  try{
...
.catch(error => {
  ...
  zokou({ nomCom: "dalle", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*powered by abuzar-MD*';
